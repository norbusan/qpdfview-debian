...
MainWindow::MainWindow(QString instanceName, QWidget* parent) : QMainWindow(parent), m_instanceName(instanceName)
{
    {
        // settings
...
void MainWindow::createDatabase()
{
    ...
    if(m_database.isOpen())
    {
        ...
        if(m_settings->value("mainWindow/restorePerFileSettings", false).toBool())
        {
...
void MainWindow::restoreTabs()
{
    ...
    if(m_database.isOpen())
    {
        ...
        QSqlQuery query(m_database);
        query.prepare("SELECT filePath,currentPage,continuousMode,layoutMode,scaleMode,scaleFactor,rotation "
                      "FROM tabs_v2 WHERE instanceName=?");
...
void MainWindow::restorePerFileSettings(DocumentView* tab)
{
    ...
    if(m_settings->value("mainWindow/restorePerFileSettings", false).toBool() && m_database.isOpen() && tab != 0)
    {
        m_database.transaction();
